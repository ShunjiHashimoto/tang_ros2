...
class Motor:
    ...
    def update_prev_value_xy(self, value_x, value_y):   
        self.prev_normalized_value_x = value_x
        self.prev_normalized_value_y = value_y

    def update_prev_value_vw(self, value_v, value_z):
        self.prev_normalized_linear_x = value_v
        self.prev_normalized_angular_z = value_z
    
    # xが前後、yが左右、左が＋
    def convert_joyinput_to_duty(self, joystick_x, joystick_y, normarized_x, normarized_y, max_duty):
        # yが前後方向、前進が＋
        # xが左右方向、右が＋
    ...
    def convert_cmdvel_to_rpm(self, linear_x, angular_z, mode):
        # cmd_velからモータのデューティ比を計算する
        corrected_angular_z = angular_z * LiDARParam.inverted if mode == "follow" else angular_z
        v_l = linear_x - corrected_angular_z * (Control.tread_width/2) 
        v_r = linear_x + corrected_angular_z  * (Control.tread_width/2)
        # 車輪の回転数に変換
